...
def assess_drivers(scan_signals: list, gbp_data: dict) -> dict:
    ...
    drivers["visibility"] = {"score": min(vis_score, 100), "band": _score_to_band(vis_score), "evidence": vis_evidence[:5]}
    ...
    drivers["trust"] = {"score": min(trust_score, 100), "band": _score_to_band(trust_score), "evidence": trust_evidence[:5]}
    ...
    drivers["authority"] = {"score": min(auth_score, 100), "band": _score_to_band(auth_score), "evidence": auth_evidence[:5]}
    
    # CONVERSION
    drivers["conversion"] = {"score": None, "band": "Insufficient evidence", "evidence": ["Conversion signals not assessed in this scan"]}
...
def identify_primary_constraint(drivers: dict) -> dict:
    ...
    for name, d in drivers.items():
        if d["band"] == "Insufficient evidence":
            continue
        candidates.append({"driver": name, "score": d["score"], "band": d["band"]})
    if not candidates:
        return {"driver": "Insufficient evidence", "confidence": "Low", "reasoning": "Insufficient data to identify a primary constraint."}
    candidates.sort(key=lambda c: c["score"])
    primary = candidates[0]
    trust = drivers.get("trust", {})
    if primary["driver"] == "trust":
        has_trust_evidence = any("reviews" in e.lower() for e in trust.get("evidence", []))
        if has_trust_evidence and len(candidates) > 1:
            primary = candidates[1]
            return {"driver": primary["driver"], "confidence": "Medium", "reasoning": f"Although on-site trust signals are limited, the business has substantial evidence of actual trust through its review profile. The real constraint appears to be {primary['driver']}."}
    if primary["driver"] == "authority":
        content_evidence = [e for e in drivers["authority"].get("evidence", []) if "content" in e.lower()]
        content_detail = content_evidence[0] if content_evidence else "Limited content depth"
        return {"driver": primary["driver"], "confidence": "High", "reasoning": f"{content_detail}. Based on available evidence, this is likely constraining growth more than other factors."}
    return {"driver": primary["driver"], "confidence": "Medium", "reasoning": f"The weakest growth driver is {primary['driver']} ({primary['band']}, {primary['score']}/100). Based on available evidence, this appears to be the most likely constraint."}
...
def build_opportunities(primary_driver: str) -> list:
    ...
    return opts.get(primary_driver, [])
...
def generate_diagnosis(scan_signals: list, gbp_data: dict, business_name: str = None) -> dict:
    ...
    for name, d in drivers.items():
        if name != primary["driver"] and d["band"] in ("Strong", "Established"):
            not_recommended.append({"recommendation": f"Major investment in {name}", "reason": f"{name.capitalize()} is already assessed as {d['band']}. Further investment likely has lower impact than addressing the primary constraint."})
    
    return {
        "business_name": business_name or "Unknown",
        "drivers": drivers,
        "primary_constraint": {"driver": primary["driver"].capitalize(), "confidence": primary["confidence"], "reasoning": primary["reasoning"]},
